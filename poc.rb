...
def parse_periode(periodes, timecard, salary)
  minutes = {}
  ...
  periodes["periode"].each do |day, periode|
    date = Date.parse(day)
...
def calculate_overtime(worked, salary)
  p "Overtime"
  overtimeWeekendHours = ((worked.sum {|_, values| values["helgetillegg"] ? values["dayMinutes"] : 0}/60.0) + (worked.sum {|_, values| values["helgetillegg"] ? values["nightMinutes"] : 0}/60.0)).round
...
def main
  ...
  minutesWorked = parse_periode(periode, timecard, salary)
